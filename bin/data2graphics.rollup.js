...
function intakeFatProportion(parrent, config) {
    ...
    var radialLine = d3$3.radialLine()
        .curve(d3$3.curveLinear)
        .context(context);

    var radialCurve = d3$3.radialLine()
        .curve(d3$3.curveCardinal.tension(0))
        .context(context);
...
function intakeFatDeviation(parrent, config) {
    ...
    // define basic location Axis
    var x = d3$3.scaleLinear()
        .domain([1 - 0.8, 6 + 0.8])
        .range([0, width]);


    var y = d3$3.scaleLinear()
        .domain([0, 7])
        .range([height, 0]);

    var xAxis = d3$3.axisBottom(x)
        .ticks(6)
        .tickSize(5)
        .tickFormat((d) => {
            return labels[d - 1]
