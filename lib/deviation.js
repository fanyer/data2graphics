...
import { detectCanvas, detectSVG } from './utils/detect'
...
export function deviation(parent, config) {
    ...
    let margin = { top: 50, right: 40, bottom: 50, left: 100 },
        width = +defaultW - margin.left - margin.right,
    ...
    function customXAxis(g) {
        g.call(xAxis).style('font-size', '20px');
    ...
    function customYAxis(g) {
        g.call(yAxis).style('font-size', '20px');
        g.select('.domain').remove();
