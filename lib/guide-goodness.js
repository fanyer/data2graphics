...
export function curveGraph(parent, config) {
    ...
    d3.range(0, 1, 0.04).forEach((e, i) => {
        ...
        let rippleRight = vBezeireArr(minus(percent(minus(JSON.parse(JSON.stringify(lineData)), 5.5), e), 5.5), 1 / 4),
            rippleLeft = vBezeireArr(percent(JSON.parse(JSON.stringify(lineData)), e), 1 / 4);

        console.log(rippleRight[0][0])


        ripple(g, 'rippleLeft', rippleLeft)
        ripple(g, 'rippleRight', rippleRight)
    })
