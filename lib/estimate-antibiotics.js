...
function init(parent, config) {
    ...
    const maskHeight = 528;


    var topPath3 = copy15('top', 'left', 'steelblue', 0, 1);
    var topPath3 = copy15('top', 'left', 'steelblue', 1, 2);
    var topPath3 = copy15('top', 'left', 'steelblue', 2, 3);
    var topPath3 = copy15('top', 'right', 'steelblue', 3, 4);
    var topPath3 = copy15('top', 'right', 'steelblue', -1, 5);
    var topPath3 = copy15('top', 'right', 'steelblue', 5, 6);
    var topPath3 = copy15('top', 'right', 'steelblue', 6, 7);
    var topPath3 = copy15('top', 'right', 'steelblue', 7, 8);

    var bottomPath4 = copy15('bottom', 'left', 'steelblue', -2, 1);
    var bottomPath5 = copy15('bottom', 'right', 'steelblue', -3, 1);
    ...
    function basicCurve(arr, postion = 'top', direction, color = 'steelblue', dx = 0, dy = 0) {
        ...
        if (direction === 'left' && postion === 'top') {
            gChild.attr('transform', 'translate(' + (-width / 2 + 40 / 2 + dx * 60) + ',' + (maskHeight - 140 - dy * 60) + ')');
        } else if (direction === 'right' && postion === 'top') {
            gChild.attr('transform', 'translate(' + (width + width / 2 - 40 / 2 + dx * 60) + ',' + (maskHeight - 80 - dy * 60) + ')');
        } else if (direction === 'left' && postion === 'bottom') {

            let x = -width / 2 + 40 / 2 + dx * 60;
            let y = maskHeight - 140 + dy * 60-800;
            ...
            let x = width + width / 2 - 40 / 2 + dx * 60;
            let y = maskHeight - 80 - dy * 60-740;
