...
function init(parent, config) {
    ...
    let gapArr = [
        (baseConf.gap[0] + 7) * 40 / 15,
        (baseConf.gap[1] + 7) * 40 / 15
    ];
    ...
    $('#centerDiv #div1').css({
        'width': 50 * (baseConf.gap[0] + 7)
    });
    $('#centerDiv #div2').css({
        'width': 50 * (baseConf.gap[1] - baseConf.gap[0] + 1) - 10,
        'border-radius': () => {
            let str = '';

            (baseConf.gap[0] === -7) && (str = '20px 0 0 20px');
            (baseConf.gap[1] === 7) && (str = '0 20px 20px 0');
            return str;
        }
    });
    $('#centerDiv #div3').css({
        'width': 50 * (7 - baseConf.gap[1])
    });


    (baseConf.gap[0] < -7) && (centerDivOne('salmon'));
    (baseConf.gap[0] > 7) && (centerDivOne('seagreen'));
    (baseConf.gap[0] <= -7) && (baseConf.gap[1] >= 7) && (centerDivOne('orange'));
    ...
    function basicCurve(arr, postion = 'top', direction = 'left', color = 'steelblue', dx = 0, dy = 0) {
        ...
        let key = locate(dx, vertical).key;
